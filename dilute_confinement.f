...
C For details see appendix C and C1
...
C**********************************************************************
      ...
      if (10000.0*(itime/10000).eq.1.0*itime.OR.itime.eq.1) then 
      ifile = itime/10000
...
!        PTS(:,T+1,:) =  0.d0 ! For boundary terms in theta
!        PPS(:,T+1,:) =  0.d0
	...
	do i = 1,P
         do j = 1,T
            do k = 1,N
             ...
             CPP(i,j,k) = pp(2,i,j)*pp(3,i,j)*c(i,j,k)
        ...
        do i = 1,P
         do j = 1,T
            do k = 1,N
             ...
             CPP(i,j,k) = ((pp(2,i,j)*pp(3,i,j))**(2.0))*c(i,j,k)
